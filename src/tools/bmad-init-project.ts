 ...
 * Creates _bmad/ directory structure, copies method files reference, registers project in Convex.
 */

import { Type } from "@sinclair/typebox";
import { mkdir, access, symlink, writeFile, lstat } from "node:fs/promises";
import { join } from "node:path";
import { readState, createProject } from "../lib/convex-state.ts";
import type { ToolResult } from "../types.ts";

export const name = "bmad_init_project";
export const description =
  "Initialize a new BMad Method project. Creates _bmad/ directory, symlinks method files, and registers in Convex. Run once per project.";
...
export async function execute(
  ...
  // Check if already initialized in Convex
  ...
  // Generate _bmad/config.yaml with project-specific values
...
`;
  ...
  // Register project in Convex
  await createProject({ projectName, projectPath, phase: "analysis" });
  ...
  return text(
    `✅ BMad project "${projectName}" initialized.\n\n` +
      `**Created:**\n` +
      `- Project registered in Convex (Mission Control)\n` +
